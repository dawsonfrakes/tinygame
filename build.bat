...
cl -Fe.bin\caveman.exe -nologo -W4 -WX -Z7 -Oi -J -EHa- -GR- -GS- -Gs0x1000000^
 caveman.c kernel32.lib user32.lib gdi32.lib opengl32.lib ws2_32.lib dwmapi.lib winmm.lib^
...
) else if "%1"=="debug" ( start remedybg .bin\caveman.exe
) else if "%1"=="doc" ( start qrenderdoc .bin\caveman.exe
) else if not "%1"=="" ( echo command '%1' not found & goto :error )

:end
del caveman.obj 2>nul
exit /b
:error
call :end
exit /b 1
